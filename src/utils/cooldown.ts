...
export async function getGmailCooldownExpiration(tenantId: string): Promise<number> {
  try {
    ...
    if (row && row.data && typeof (row.data as any).expiration === 'number') {
      return (row.data as any).expiration;
    }
  } catch (err) {
    console.error('Failed to get Gmail cooldown expiration:', err);
  }
  return 0;
}
...
export async function setGmailCooldown(tenantId: string, durationMs: number = 20 * 60 * 1000): Promise<void> {
  try {
    ...
    console.log(`⏳ [Cooldown] Set Gmail API cooldown for tenant ${tenantId} until ${new Date(expiration).toISOString()}`);
  } catch (err) {
    console.error('Failed to set Gmail cooldown:', err);
  }
}
