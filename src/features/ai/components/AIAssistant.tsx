...
import { ChevronRight, ArrowUp, Pause, X, Brain, Wrench, Cpu, History, Plus, MessageSquare } from 'lucide-react';
import { useChatStore, ChatMessage } from '@/features/ai/store/chatStore';
...
export default function AIAssistant({ user, projectName: _projectName }: AIAssistantProps) {
    ...
    return (
        <>
            ...
            <aside
                style={{ width: isRightSidebarCollapsed ? '0px' : (isMobile ? '100%' : `${sidebarWidth}px`) }}
                className={`bg-sidebar-bg flex flex-col justify-between select-none shrink-0 ${isResizing ? 'transition-none' : 'transition-all duration-300'
                    } ${isRightSidebarCollapsed
                        ? 'border-l-0 overflow-hidden relative z-30 w-0'
                        : (isMobile ? 'fixed inset-0 z-[100] w-full h-full bg-sidebar-bg border-l-0' : 'border-l border-border relative z-30 min-w-[280px] max-w-[600px]')
                    }`}
            >
                ...
                {/* Toggle Collapse button (only when expanded) */}
                {!isRightSidebarCollapsed && !isMobile && (
                    <button
